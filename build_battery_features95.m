function [ X ] = build_battery_features95( batch )
...
for i = 1:numBat
    ...
    X(i,2) = max(batch(i).summary.QDischarge(1:95))...
        - batch(i).summary.QDischarge(2);
    %capacity at cycle 100
    X(i,3) = batch(i).summary.QDischarge(95);
    
    %Linear fit of Q v. N
    R3 = regress(batch(i).summary.QDischarge(2:95),[2:95;ones(1,length(2:95))]');
    X(i,4) = R3(1);
    X(i,5) = R3(2);
    
    %Linear fit of Q v. N, only last 10 cycles
    R1 = regress(batch(i).summary.QDischarge(91:95),[91:95;ones(1,length(91:95))]');
    X(i,6) = R1(1);
    X(i,7) = R1(2);
    
    %Q features
    QDiff = batch(i).cycles(95).Qdlin - batch(i).cycles(10).Qdlin;
    ...
    E95 = trapz(batch(i).cycles(95).Qdlin,batch(i).Vdlin);
    X(i,15) = log10(E10-E95);
