...
for i = 1:numBat
    ...
    C1(i) = str2num(batch(i).policy_readable(1:idx(1)));
    C2(i) = str2num(batch(i).policy_readable(idx(1):idx(2)));
    C3(i) = str2num(batch(i).policy_readable(idx(2):idx(3)));
    C4(i) = str2num(batch(i).policy_readable(idx(3):idx(4)));
...
M = [C1, C2, C3, C4, round(ypred), round(ypred_l), round(ypred_h), ...
    lifetimes, channels, barcode, policies];
